...
class MWDModelSubmissions_mwd {
	...
	public function get_labels_parameters($form_id) {
		...
		$where_labels = array();
		$where2 = array();
		$order_by = ((isset($_POST['order_by']) && sanitize_text_field(stripslashes($_POST['order_by'])) != '') ? sanitize_text_field(stripslashes($_POST['order_by'])) : 'group_id');
		...
		$lists['hide_label_list'] = MWD_Library::get('hide_label_list', '');
		$lists['startdate'] = MWD_Library::get('startdate', '');
		$lists['enddate'] = MWD_Library::get('enddate', '');
		$lists['ip_search'] = MWD_Library::get('ip_search', '');

		$lists['username_search'] = MWD_Library::get('username_search', '');
		$lists['useremail_search'] = MWD_Library::get('useremail_search', '');
		$lists['id_search'] = MWD_Library::get('id_search', '');
		...
		foreach ($labels_id as $key => $label_id) {
			if (in_array($label_id, $labels)) {
				...
				if (isset($_POST[$form_id . '_' . $label_id . '_search'])) {
					$search_temp = sanitize_text_field($_POST[$form_id . '_' . $label_id . '_search']);
				}
				else {
					$search_temp = '';
				}
	...
	public function sort_group_ids($sorted_label_names_count, $group_ids) {
		...
		$group_id_s = array();
		$l = 0;
		if (is_array($group_ids) && count($group_ids) > 0 && $count_labe) {
