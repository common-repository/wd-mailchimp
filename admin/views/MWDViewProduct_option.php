...
class MWDViewProduct_option {
	...
	public function display() {
		...
		$property_id = ((isset($_GET['property_id'])) ? sanitize_text_field($_GET['property_id']) : '-1');
		?>
		<style>
		#mwd-paypal-properties {
			padding:0 10px;
		...
		<div id="mwd-paypal-properties">
			<div class="main">
				<span>Properties</span>
				<button class="mwd-save" onclick="save_options(); return false;">
					Save
					<span></span>
				</button>
			</div>
			<div class="mwd-properties">
				<div>
					<label class="mwd-property-label">Type </label>
					<select id="option_type" class="mwd-property-value" onchange="type_add_predefined(this.value)">
				...
				<div>
					<label class="mwd-property-label" style="margin-right: 24px;">Title </label>
					<input type="text" class="mwd-property-value" id="option_name"/>
				</div>
				<div>
					<label class="mwd-property-label">Properties</label> &nbsp;
					<img id="el_choices_add" src="<?php echo MWD_URL . '/images/add.png?ver='. get_option("mwd_version").''; ?>" style="vertical-align:middle; cursor: pointer;" title="add" onclick="add_choise_option()" />
				</div>
		...
		function save_options() {
			...
			if ($property_id == '-1') {
				...
				var li_edit = document.createElement('a');
					li_edit.setAttribute("onclick","tb_show('', 'admin-ajax.php?action=product_option&field_id=<?php echo $id; ?>&property_id=" + i + "&width=530&height=370&TB_iframe=1')");
					li_edit.setAttribute("class", "thickbox-preview" + i);
				var li_edit_img = document.createElement('img');
					li_edit_img.setAttribute("src", "<?php echo MWD_URL . '/images/edit.png?ver='. get_option("mwd_version").''; ?>");
					li_edit_img.style.cssText = "margin-left: 13px; width: 17px; cursor:pointer;";
					li_edit.appendChild(li_edit_img);
				var li_x = document.createElement('img');
					li_x.setAttribute("src", "<?php echo MWD_URL . '/images/delete.png?ver='. get_option("mwd_version").''; ?>");
					li_x.setAttribute("onClick", 'remove_property(<?php echo $id; ?>,' + i + ')');
					li_x.style.cssText = "margin-left: 3px; cursor: pointer; width: 22px;";
		...
		function type_add_options(w_choices) {
			...
			for (j = 0; j < n; j++) {
				...
				var el_choices_remove = document.createElement('img');
					...
					el_choices_remove.setAttribute("src", "<?php echo MWD_URL . '/images/delete.png?ver='. get_option("mwd_version").''; ?>");
