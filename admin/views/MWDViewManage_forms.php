...
class MWDViewManage_forms {
	...
	public function display() {
		...
		$search_value = MWD_Library::get('search_value', '');
		$search_type = MWD_Library::get('search_select_value_type', '0');
		$search_list = MWD_Library::get('search_select_value_list', '0');
	...
	public function edit($id) {
		...
		$list_ids = isset($_GET['list_id']) ? explode(',', sanitize_text_field($_GET['list_id'])) : ($row->list_id ? explode(',', $row->list_id) : array());
		...
		$form_type = isset($_GET['form_type']) ? sanitize_text_field($_GET['form_type']) : $row->type;
		$mergeParams = array();
		$groupParams = array();
		...
		wp_enqueue_script('mwd_forms', MWD_URL . '/js/mwd_forms.js', array(), get_option("mwd_version"));
		?>
