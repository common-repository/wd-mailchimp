...
class MWDViewSubmissions_mwd {
	...
	public function display($form_id) {
		if(get_option('mwd_api_validation') != 'valid_apikey') {
			echo MWD_Library::message("You need to connect to MailChimp before you can start creating forms. Head over to the <a href='".add_query_arg(array('page' => 'manage_mwd'), admin_url('admin.php'))."'>Mailchimp by 10Web</a> and enter your API key.", 'error');
			die();
		}
		global $wpdb;
		$forms = $this->model->get_form_titles();
		$statistics = $this->model->get_statistics($form_id);
		$labels_parameters = (array) $this->model->get_labels_parameters($form_id);
		...
		$order_by = MWD_Library::get('order_by', 'group_id');
		...
		$ispaypal = FALSE;
		$temp = array();
		$m = is_array($sorted_label_names) ? count($sorted_label_names) : 0;
		$n = is_array($rows) ? count($rows) : '';
		$group_id_s = array();	
		$group_id_s = $this->model->sort_group_ids(is_array($sorted_label_names) ? count($sorted_label_names) : 0 ,$group_ids);
		...
		function remove_all() {
			...
			<?php
			$n = is_array($rows) ? count($rows) : '';
      if( is_array($sorted_label_names) || is_object($sorted_label_names) ) {
					...
					<table class="wp-list-table widefat fixed posts table_content">
						<thead>
							...
							<tr id="fields_filter" style="display: none;">
								...
								<?php
                if( is_array($sorted_label_names) ) {
                  for ( $i = 0; $i < count($sorted_label_names); $i++ ) {
                    $styleStr = $this->model->hide_or_not($lists['hide_label_list'], $sorted_labels_id[$i]);
	...
	public function show_stats($form_id) {
		$key = MWD_Library::get('sorted_label_key', '');
